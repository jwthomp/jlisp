...
{
	unsigned long exp = p_vm->m_exp;
	while(exp) {
		p_vm->m_exec_stack[exp] = p_vm->m_exec_stack[exp - 1];
		exp--;
	}
	p_vm->m_exp++;
	...
	p_vm->m_exec_stack[0] = value_create_cons(p_vm, p_closure, state);

printf("insert_exec: ip: %d exp: %lu\n", p_vm->m_ip, p_vm->m_exp);
